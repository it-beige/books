  ...
  <div class="edit-table-box">
    <div class="top-tool-box">
      ...
      <div v-if="controlBar.length>0" class="right-tool">
        ...
        <el-tooltip
          v-if="controlBar.includes('delete')"
          effect="dark"
          content="删除"
          placement="bottom"
        >
        ...
        <el-tooltip
          v-if="controlBar.includes('refresh')"
          effect="dark"
          content="刷新"
          placement="bottom"
        >
        ...
        <el-tooltip
          v-if="controlBar.includes('column')"
          effect="dark"
          content="选择显示列"
          placement="bottom"
        >
        ...
        <el-tooltip
          v-if="controlBar.includes('search')"
          effect="dark"
          content="隐藏搜索框"
          placement="bottom"
        >
...
export default {
  ...
  props: {
    ...
    enableValidation: {
      // 是否开启数据校验 （enableEdit = true时生效）
      type: Boolean,
      default: true
    },
    controlBar: {
      // 表格的操作按钮  可选值['edit', 'delete', 'refresh','column', 'search']
      type: Array,
      default: () => []
    }
  },
  ...
  data() {
    return {
      ...
      transData: [], // 总的列
      currentPage: 1, // 当前页码
      pageSizes: this.pageInfo.pageSizes
        ? this.pageInfo.pageSizes.sort((x, y) => x - y)
        : [10, 20, 50, 100, 200], // 备选每页显示多少条数据
  ...
  methods: {
    ...
    handleControlBar(sign) {
      ...
      if (that.selectionData.length) {
        if (sign === 'delete') {
          MessageBox.confirm('确定删除选中的数据吗？', '提示', {
